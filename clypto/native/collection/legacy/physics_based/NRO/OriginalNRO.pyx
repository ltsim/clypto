...
cdef class OriginalNRO(_LegacyOptimizer):
    """
    ...
    Examples
    ~~~~~~~~
    >>> from clypto.native.collection.legacy.physics_based import NRO    >>> import numpy as np
    ...
    def evolve(self, epoch):
        ...
        for idx in range(self.pop_size):
            ...
            if self.generator.uniform() <= Pfi:
                ...
                if self.generator.uniform() <= Pb:
                    ...
                    gauss = np.array(
                        [
                            self.generator.normal(self.g_best.solution[j], xichma1[j])
        ...
        for idx in range(self.pop_size):
            ...
            if (ranked_pop[idx] * 1.0 / self.pop_size) < self.generator.random():
                ...
                _, _, worst = self.get_special_agents(
                    self.pop, n_worst=1, minmax=self.problem.minmax
                )
                X_worst = worst[0]
                for j in range(self.problem.n_dims):
                    ##### Based on Eq. 21
