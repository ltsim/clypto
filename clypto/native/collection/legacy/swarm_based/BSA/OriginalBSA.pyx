...
cdef class _OriginalBSAAgent(_LegacyAgent):
...
cdef class OriginalBSA(_LegacyOptimizer):
    """
    ...
    Examples
    ~~~~~~~~
    >>> from clypto.native.collection.legacy.swarm_based import BSA    >>> import numpy as np
    ...
    def __init__(
        ...
        **kwargs: object
        ...
        self.set_parameters(
            ["epoch", "pop_size", "ff", "pff", "c1", "c2", "a1", "a2", "fc"]
        )
    ...
    def generate_empty_agent(self, solution: np.ndarray | None = None) -> _LegacyAgent:
        ...
        return _OriginalBSAAgent(solution=solution, local_solution=local_position)
        ...
        if epoch % self.ff != 0:
            ...
            for idx in range(0, self.pop_size):
                ...
                if self.generator.uniform() < prob:  # Birds forage for food. Eq. 1
                    ...
                    A1 = self.a1 * np.exp(
                        -self.pop_size
                        * self.pop[idx].local_target.fitness
