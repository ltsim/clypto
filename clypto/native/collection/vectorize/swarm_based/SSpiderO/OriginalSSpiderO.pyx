...
cdef class OriginalSSpiderO(AgentListOptimizer):
    """
    ...
    Examples
    ~~~~~~~~
    >>> from clypto.native.collection.vectorize.swarm_based import SSpiderO    >>> import numpy as np
    ...
    cdef public object fp_max
    cdef public object fp_min
    cdef public object n_f
    cdef public object n_m
    cdef public object p_m
    cdef public object pop_males
    cdef public object pop_females
    ...
    def __init__(
        ...
        *,
        ...
        LegacyNativeOptimizer.__init__(
            self,
            parameters=["epoch", "pop_size", "fp_min", "fp_max"],
            sort_flag=False,
            parallelizable=True,
            name=name,
            mode=mode,
    ...
    def mating__(self):
        ...
        pop_males_new = [
            ...
            if self.pop_males[idx].weight > my_median
        ]
        ...
        for idx in range(0, len(pop_males_new)):
            for jdx in range(0, self.n_f):
                dist = np.linalg.norm(
                    pop_males_new[idx].solution - self.pop_females[jdx].solution
                )
                if dist < r:
    ...
    def evolve_agents(self, epoch):
        ...
        self.objs = self.recalculate_weights__(pop)
