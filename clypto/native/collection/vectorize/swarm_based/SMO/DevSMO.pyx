...
cdef class DevSMO(AgentListOptimizer):
    """
    ...
    Examples
    ~~~~~~~~
    >>> from clypto.collection.swarm_based import SMO    >>> import numpy as np
    ...
    cdef public object max_groups
    cdef public object perturbation_rate
    cdef public object num_groups
    cdef public object group_size
    cdef public object LLL
    cdef public object GLL
    cdef public object local_limit_counts
    cdef public object global_limit_count
    cdef public object groups
    cdef public object local_leaders
    ...
    def local_leader_phase(self):
        for group_idx, group in enumerate(self.groups):
            ...
            for idx in range(n_items):
                list_rand = self.generator.choice(
                    list(set(range(n_items)) - {idx}),
            ...
            while attempts < self.group_size:
                for idx, agent in enumerate(group):
                    ...
                    if self.generator.uniform(0, 1) < prob[prob_idx]:
                        ...
                        jdx = self.generator.choice(
                            list(set(range(self.group_size)) - {idx})
                        )
