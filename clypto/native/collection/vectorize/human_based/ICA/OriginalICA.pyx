...
cdef class OriginalICA(AgentListOptimizer):
    """
    ...
    cdef public object empire_count
    cdef public object assimilation_coeff
    cdef public object revolution_prob
    cdef public object revolution_rate
    cdef public object revolution_step_size
    cdef public object zeta
    cdef public object n_revoluted_variables
    cdef public object pop_empires
    cdef public object pop_colonies
    cdef public object empires
    ...
    def __init__(
        ...
        *,
        ...
        self.empire_count = cy.validator(int, empire_count, [2, 2 + int(self.pop_size / 5)], "empire_count")
        self.assimilation_coeff = cy.validator(float, assimilation_coeff, [1.0, 3.0], "assimilation_coeff")
        self.revolution_prob = cy.validator(float, revolution_prob, (0, 1.0), "revolution_prob")
        self.revolution_rate = cy.validator(float, revolution_rate, (0, 1.0), "revolution_rate")
        self.revolution_step_size = cy.validator(float, revolution_step_size, (0, 1.0), "revolution_step_size")
        self.zeta = cy.validator(float, zeta, (0, 1.0), "zeta")
    ...
    def revolution_country__(
        ...
        idx_list = self.generator.choice(
            range(0, self.problem.n_dims), n_revoluted, replace=False
        )
        if len(idx_list) == 0:
            idx_list = np.append(
                idx_list, self.generator.integers(0, self.problem.n_dims)
            )
    ...
    cdef void initialization(self):
        ...
        self.n_revoluted_variables = int(
            round(self.revolution_rate * self.problem.n_dims)
        )
        # pop = Empires
        colony_count = self.pop_size - self.empire_count
        self.pop_empires = [agent.copy() for agent in self.objs[: self.empire_count]]
        ...
        for i in range(0, self.empire_count - 1):
            ...
            idx_list = self.generator.choice(
                list(set(range(0, colony_count)) - set(idx_already_selected)),
                n_colonies,
                replace=False,
            ).tolist()
    ...
    def evolve_agents(self, epoch):
        ...
        self.objs = self.pop_empires + self.pop_colonies
