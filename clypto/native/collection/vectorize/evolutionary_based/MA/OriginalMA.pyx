...
cdef class OriginalMA(AgentListOptimizer):
    """
    ...
    cdef public object p_local
    cdef public object max_local_gens
    cdef public object bits_per_param
    ...
    def _generate_empty_agent(self, solution: np.ndarray | None = None) -> LegacyAgent:
        ...
        return FieldAgent(solution=solution, bitstring=bitstring)
    ...
    def _evolve_agents(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            if self.generator.random() < self.p_local:
                self.objs[idx] = self.bits_climber__(pop[idx])
