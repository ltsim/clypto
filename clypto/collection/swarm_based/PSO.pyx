...
cdef class _LDW_PSOAgent(_LegacyAgent):
    cdef public object velocity
    cdef public object local_solution
    cdef public object local_target
    def __init__(self, solution=None, target=None, velocity=None, local_solution=None, local_target=None):
...
cdef class _CL_PSOAgent(_LegacyAgent):
    cdef public object velocity
    cdef public object local_solution
    cdef public object local_target
    def __init__(self, solution=None, target=None, velocity=None, local_solution=None, local_target=None):
...
cdef class LDW_PSO(_LegacyOptimizer):
    """
...
cdef class CL_PSO(_LegacyOptimizer):
    """
    ...
    def evolve(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            ...
            for jdx in range(0, self.problem.n_dims):
                if self.generator.random() > pci:
                    ...
                    if self.compare_target(
                        self.pop[id1].target, self.pop[id2].target, self.problem.minmax
                    ):
                        vj = wk * self.pop[idx].velocity[
                            jdx
                        ] + self.c_local * self.generator.random() * (
        ...
        if self.mode in self.AVAILABLE_MODES:
            ...
            pop_child = self.greedy_selection_population(
